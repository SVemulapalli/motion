...
import { resolveMotionNumber } from "../utils/resolveMotionValue"
import { MotionRect } from "../presentation/Frame/types"
...
export interface ConstraintMotionProperties extends Partial<WithFractionOfFreeSpace> {
    parentSize: Size | AnimatableObject<Size> | null
...
export namespace ConstraintMotionValues {
    // Returns concrete current values given some ConstraintProperties.
    export const fromProperties = (
        props: Partial<ConstraintMotionProperties> & { autoSize?: boolean }
    ): UserConstraintValues => {
        const { left, right, top, bottom, width, height, centerX, centerY, aspectRatio, autoSize } = props
        const constraints = ConstraintMask.quickfix({
            left: isFiniteNumber(left) || isMotionValue(left),
            right: isFiniteNumber(right) || isMotionValue(right),
            top: isFiniteNumber(top) || isMotionValue(top),
            bottom: isFiniteNumber(bottom) || isMotionValue(bottom),
            widthType: valueToDimensionType(width),
            heightType: valueToDimensionType(height),
            aspectRatio: aspectRatio || null,
            fixedSize: autoSize === true,
        ...
        if (constraints.widthType !== DimensionType.FixedNumber && typeof width === "string") {
            ...
            widthValue = resolveMotionNumber(width)
        }
        ...
        if (constraints.heightType !== DimensionType.FixedNumber && typeof height === "string") {
            ...
            heightValue = resolveMotionNumber(height)
        }
        ...
        return {
            left: constraints.left ? resolveMotionNumber(left) : null,
            right: constraints.right ? resolveMotionNumber(right) : null,
            top: constraints.top ? resolveMotionNumber(top) : null,
            bottom: constraints.bottom ? resolveMotionNumber(bottom) : null,
            widthType,
            heightType,
            width: widthValue,
            height: heightValue,
    ...
    export const toMinSize = (
        ...
        const parentWidth = parentSize ? resolveMotionNumber(parentSize.width) : null
        const parentHeight = parentSize ? resolveMotionNumber(parentSize.height) : null
