...
import { FrameSpecificProps, FrameProperties } from "../render/presentation/Frame"
...
export enum PageContentDimension {
    Auto = "auto",
    Stretch = "stretch",
}
const pageContentDimensionOptions: PageContentDimension[] = [PageContentDimension.Auto, PageContentDimension.Stretch]
const pageContentDimensionTitles = pageContentDimensionOptions.map(option => {
    switch (option) {
        case PageContentDimension.Auto:
            return "Auto"
        case PageContentDimension.Stretch:
            return "Stretch"
    }
})
...
const pageEffectOptions: PageEffectDefault[] = [
    PageEffectDefault.None,
    PageEffectDefault.CoverFlow,
    PageEffectDefault.Cube,
    PageEffectDefault.Wheel,
    PageEffectDefault.Pile,
]
const pageEffectTitles = pageEffectOptions.map(option => {
    switch (option) {
        case PageEffectDefault.None:
            return "None"
        case PageEffectDefault.Cube:
            return "Cube"
        case PageEffectDefault.CoverFlow:
            return "Cover Flow"
        case PageEffectDefault.Wheel:
            return "Wheel"
        case PageEffectDefault.Pile:
            return "Pile"
    }
})
...
export type PageEffectFunction = (info: PageEffectInfo) => Partial<FrameProperties>
 ...
 * @public
 */
export class Page extends React.Component<Partial<PageProperties>> {
    ...
    private static pageProps: PageSpecificProps & Partial<ScrollProps> = {
        direction: "horizontal",
        contentWidth: PageContentDimension.Stretch,
        contentHeight: PageContentDimension.Stretch,
        ...
        defaultEffect: PageEffectDefault.None,

        background: "transparent",
    }

    static defaultProps: PageProperties = Object.assign({}, CoreFrame.defaultProps, Scroll.scrollProps, Page.pageProps)
    ...
    static propertyControls: PropertyControls<PageProperties> = {
        ...
        alignment: {
            ...
            hidden(props) {
                ...
                return crossDimension === PageContentDimension.Stretch
            },
        },
        gap: {
    ...
    private boundedCurrentPage(props: PageProperties): number {
     ...
     * @internal
     */
    componentWillUpdate(nextProps: PageProperties) {
    ...
    render() {
        ...
        const crossDimension = isHorizontalDirection ? contentHeight : contentWidth
        const alignment =
            crossDimension === PageContentDimension.Stretch ? PageContentDimension.Stretch : this.properties.alignment
        ...
        const childComponents = React.Children.map(children, (child, index) => {
            ...
            if (mainDimension === PageContentDimension.Stretch) {
...
function paddingFromProps(props: PageProperties) {
...
function defaultEffects(type: PageEffectDefault): PageEffectFunction | null {
    switch (type) {
        case PageEffectDefault.Cube:
            return cubeEffect
        case PageEffectDefault.CoverFlow:
